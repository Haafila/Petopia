...
// POST /api/reports/generate - Generate reports based on filters and options
router.post('/generate', async (req, res) => {
  try {
    ...
    // Fetch data from database based on query and report type
    let data;
    try {
      switch (type) {
        case 'orders':
    ...
    try {
      switch (format) {
        ...
        case 'csv':
          return generateCSVReport(data, type, options, res);
        case 'excel':
...
// Fetch Orders Data - Adapted for your schema
...
// Fetch Revenue Data - Adapted for your schema
...
// Fetch Product Sales Data - Adapted for your schema
...
// Generate PDF Report with enhanced styling
...
// Add Orders PDF Content with enhanced styling
...
// Add Revenue PDF Content with enhanced styling
function addRevenuePDFContent(doc, data, colors) {
  ...
  // Summary box
  doc.rect(50, y + 10, doc.page.width - 100, 100)
  .fill(colors.primary);

  doc.fillColor(colors.text)
  .font('Courier-Bold')
  .fontSize(14)
  .text('REVENUE SUMMARY', doc.page.width / 2, y + 20, { align: 'center' });
  ...
  doc.fontSize(12)
    .text(`Total Revenue:`, 100, y + 45)
    .font('Courier-Bold')
    .text(`LKR ${totalRevenue}`, doc.page.width - 150, y + 45, { align: 'right' })
    .font('Courier')
    .text(`Total Orders:`, 100, y + 65)
    .font('Courier-Bold')
    .text(`${totalOrders}`, doc.page.width - 150, y + 65, { align: 'right' })
    .font('Courier')
    .text(`Overall Avg Order Value:`, 100, y + 85)
    .font('Courier-Bold')
    .text(`LKR ${(totalRevenue / totalOrders).toFixed(2)}`, doc.page.width - 100, y + 85, { align: 'right' });
  }
}

// Add Products PDF Content with enhanced styling
function addProductsPDFContent(doc, products, colors) {
  // Table header
  const tableTop = 150;
  const columns = [
    { header: 'Product Name', width: 150 },
    { header: 'Unit Price', width: 80 },
    { header: 'Quantity Sold', width: 100 },
    { header: 'Total Revenue', width: 100 }
  ];
  ...
  products.forEach((product, i) => {
    ...
    doc.fillColor(colors.text)
       .fontSize(10)
       .font('Courier')
       .text(product.name, 55, y, { width: 145 })
       .text('LKR ' + product.unitPrice, 285, y)
       .text(product.quantitySold.toString(), 365, y)
       .text('LKR ' + product.totalRevenue, 465, y);
  ...
  // Summary box with gradient fill
  doc.rect(430, y + 10, 310, 90)
     .fill(colors.primary);
     
  doc.fillColor(colors.text)
     .font('Courier-Bold')
     .fontSize(14)
     .text('PRODUCT SALES SUMMARY', 400, y + 20, { align: 'center' });
     
  if (products.length > 0) {
    const totalRevenue = products.reduce((sum, product) => sum + parseFloat(product.totalRevenue), 0).toFixed(2);
    const totalQuantity = products.reduce((sum, product) => sum + product.quantitySold, 0);
    
    doc.fontSize(12)
       .text(`Total Products:`, 260, y + 40)
       .font('Courier-Bold')
       .text(`${products.length}`, 540, y + 40, { align: 'right' })
       .font('Courier')
       .text(`Total Units Sold:`, 260, y + 60)
       .font('Courier-Bold')
       .text(`${totalQuantity}`, 540, y + 60, { align: 'right' })
       .font('Courier')
       .text(`Total Revenue:`, 260, y + 80)
       .font('Courier-Bold')
       .text(`LKR ${totalRevenue}`, 540, y + 80, { align: 'right' });
  }
}

// Helper function to draw styled table header
...
// Generate CSV Report
async function generateCSVReport(data, type, options, res) {
  try {
    ...
    switch (type) {
      case 'orders':
        headers = [
          ...
          { id: 'deliveryCity', title: 'Delivery City' }
        ];
        break;
      case 'revenue':
        headers = [
      ...
      case 'products':
        ...
        break;
      default:
        return res.status(400).json({ message: 'Invalid report type for CSV' });
    }
    
    // Create a temporary file path (just for creating the file, will be streamed directly)
    ...
    // Handle error case to ensure file cleanup
    fileStream.on('error', (err) => {
      ...
      try {
        ...
        console.error('Error cleaning up CSV file after stream error:', cleanupError);
      }
      if (!res.headersSent) {
...
async function generateExcelReport(data, type, options, res) {
  ...
  if (type === 'orders') {
    worksheet.getColumn('total').numFmt = '"LKR"#,##0.00';
  } else if (type === 'revenue') {
    worksheet.getColumn('revenue').numFmt = '"LKR"#,##0.00';
    worksheet.getColumn('averageOrderValue').numFmt = '"LKR"#,##0.00';
  } else if (type === 'products') {
    worksheet.getColumn('unitPrice').numFmt = '"LKR"#,##0.00';
    worksheet.getColumn('totalRevenue').numFmt = '"LKR"#,##0.00';
  }
